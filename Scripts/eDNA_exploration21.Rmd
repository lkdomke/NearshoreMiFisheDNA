...
# Read in data
```{r}
edna <- read.csv("Data/eDNA_tax_filtered_3-24-22.csv")
#loc <- read.csv("../../Chp3_SeascapeDynamics/Data/chp3_sites.csv")
#seine <- read.csv("/Users/liadomke/Desktop/Grad School/UAF/Data/Chp3_SeascapeDynamics/Data/fish_beach_seine_2021_1-5-21.csv")

edna.minus.control <- read.csv("Data/eDNA_tax_filtered_minus_controls_4-15-22.csv", stringsAsFactors = FALSE, header = TRUE)
```
...
p1.sites <- p1 + facet_wrap(~sites)

#ggsave(p1.sites, filename = "../Images/eDNA_plots/eDNA_reads_by_techsample.png")
```
...
p2 <- edna.red.hab %>%
  ...
  geom_col(aes(x = field_tech_rep, y = count, fill = taxon)) + plot_theme() +
  theme(axis.text.x = element_text(angle = 45))
p2.sites <- p2 + facet_wrap(~sites)

#ggsave(p2.sites, filename = "../Images/eDNA_plots/eDNA_reads_by_techsample_kelp.png")
```
