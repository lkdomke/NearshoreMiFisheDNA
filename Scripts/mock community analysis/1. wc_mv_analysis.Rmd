...
## ISA sp in seine w/ abund
```{r}
head(seine.corrected2)
head(heatmapdat)
seine.sp <- filter(heatmapdat, nested == "Both gears" | nested == "Seine") %>%
  distinct(rn)

# create a few groups for 
s.eel <- seine.corrected2 %>%
  filter(corrected_sp %in% seine.sp$rn,
         type == "seine", 
         habitat == "eelgrass") %>%
...
abundheatdat<- rbind(s.eel, s.mixed, s.kelp, s.kelpmixed, s.eelmixed, s.all) %>%
   mutate(comb = factor(comb, levels = c("Seine: eelgrass", "Seine: mixed", "Seine: kelp",
                                        "Seine: eelgrass & kelp", "Seine: eelgrass & mixed",
                                        "Seine: kelp & mixed", "Seine: all habitats")),
          corrected_sp = factor(corrected_sp, levels = c("Artedius spp.","Hexagrammos",
                                                         ...
                                                         "Anoplarchus", "Sebastes 1","Aulorhynchus flavidus", 
                                                         "Hemilepidotus spp.", "Apodichthys flavidus",
                                                         "Blepsias cirrhosus", "Ophiodon elongatus",
                                                         "Brachyistius frenatus", "Oxylebius pictus")))
