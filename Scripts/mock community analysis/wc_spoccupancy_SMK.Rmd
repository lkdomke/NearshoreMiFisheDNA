...
## Dummy detection covariates - not run
...
data.list <- list(y = y.full,
                  ...
                  #det.covs = det.covs, # removing this now because doy was dummy and we're running intcept only m odel
                  sites = sites.index,
                  species = sp.indx)
...
*Note as in vignette posterior checks haven't been implemented for integrated multispecies models yet, for now I'm going to see if I can implement Shannon's 2-stage code and then come back to this*
```{r}
# ppc.out <- ppcOcc(out.1, fit.stat = "freeman-tukey", group = 1)
# summary(ppc.out)
# simultating data does work
# simIntMsOcc
```
## inclusions of region?
...
summary(out.1$ESS$alpha.comm) # min 396, max 510, okay
summary(out.1$ESS$beta.comm) # min 390, max 1150 okay
summary(out.1$ESS$tau.sq.beta) # min 325, max 593 (getting low)
summary(out.1$ESS$tau.sq.alpha) # min 375, max 1023 okay
summary(out.1$ESS$beta) # min 197 (don't love this), max 5033 SP level occurrence coef
summary(out.1$ESS$alpha) # min 321, max 6292
```
...
mcmc_trace(out.1$tau.sq.beta.samples) # NOT GOOD; how do we fix this? 
```


## Posterior predictive checks w/ simulated data
```{r}
hist(out.1$beta.samples)
posterior.beta <- out.1$beta.samples
str(posterior.beta)

predict(out.1)

simIntMsOcc(posterior.beta)

head(out.1$beta.comm.samples)
str(out.1$beta.comm.samples)
```
...
Shannon: For mapping you'd need some spatial data (points, polygons, or raster files) for all of your occupancy covariates)
....
...
``` {r}
# set.seed(8420)
# PH.covs <- data.frame(size = as.factor(sample(1:3, nSpecies, replace = T)),
#                       behavior = as.factor(sample(c("a","b","c"), nSpecies, replace = T))); PH.covs
...
plogis(-4.5) # inverse logit, 0.01098
exp(-4.5) # 0.01109
```
...
str(post.enda$beta.samples)
str(post.seine$beta.samples)
summary(post.edna$beta.samples)
...
#exp(-4.5595)  = 0.01046
# so after accounting for presence/occupancy the odds of detecting a fish with plates compared to the reference level of a fish (facultative, with deciduous scale, both pelagic and benthic) then its 0.01 as likely as detecting the ref level fish?
