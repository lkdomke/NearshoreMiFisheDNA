...
site.region2 <- site.region %>%
  dplyr::select(bay_year, bay_year2, region)

sites <- sites %>%
  left_join(site.region2)
```
...
canada.crop2 <- st_transform(canada, crs = st_crs(26908))

# plot(canada.crop2$geometry)
# drawExtent()
inset <- ggplot() +
  geom_sf(ak.small, mapping = aes(), fill = "#999999", color = "black") +
  geom_sf(canada.crop2, mapping = aes(), fill = "#5e5e5e", color = "black") +
  coord_sf(xlim = c(-68204, 1149204), ylim = c(5646690, 6662975)) +
  ...
  geom_text(mapping = aes(x = 200000, y = 6233075), label = "Southeast \n Alaska", size = 5) +
  geom_text(mapping = aes(x = 899556.3, y = 6421835), label = "Canada", size = 5) + #7021835
  theme_classic() +
...
main_map <- ggplot() +
  ...
  geom_sf(ak.crop, mapping = aes(), fill = "#999999") +
  geom_sf(sites.26908, mapping = aes(fill = habitat, shape = region#, size = habitat
                                     ), 
          color = "black", size = 4) +
  ...
  theme(legend.position = "inside",
        legend.position.inside = c(0.12,0.24),
        #legend.text = element_markdown(size = 15),
        legend.spacing.y = unit(0, "cm"),
        axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +
  annotation_scale(text_family = "Calibri", text_cex = 1.5, line_width = 2) +
  annotation_north_arrow(which_north = "true", location = "bl", height = unit(2, "cm"),
                         pad_x = unit(7,"cm"), pad_y = unit(1, "cm")) +
...
final <- main_map + inset_element(inset, 0.66, 0.66, 1, 1, align_to = "plot", ignore_tag = TRUE)
# final
# ggsave(filename = "Images/map_figure_adj_6-25-25.png",
#        plot = final,
#        height = 16,
#        width = 12)
```
