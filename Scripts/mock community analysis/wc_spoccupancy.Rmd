...
We can check a couple things as we wait for this to run, the acceptance values should be generally between 40 - 60. If you see it consistently at 0 or 100 then thats bad and you should go look at that youtube video (linked below for direction)

When looking at the summary the Rhat should be less than 1.1 if not then  you likely need to increase the burn in? 
```{r}
...
Based off of the youtube video looking at simple single species models we have a couple other posterior summaries checks we can do (see here https://www.youtube.com/watch?v=arYqlfs6lIQ)

```{r}
# Can look at the bayesian p-value and investigate goodness of fit check with 
# ppc.out <- ppcOcc(out.1, fit.stat = "freeman-tukey", group = 1)
# summary(ppc.out) 
# Not possible yet, under active development

# for posterior summaries
# look at objects within resulting object
names(out.1)
str(out.1$beta.samples)

# create simple plot summaries using MCMCvis package
# occupancy covariate community effects ---- 
MCMCplot(out.1$beta.comm.samples, ref_ovl = T, ci = c(50,95))
# detection covariate effects ----
MCMCplot(out.1$alpha.comm.samples, ref_ovl = T, ci = c(50, 95))
MCMCplot(out.1$alpha.samples, ref_ovl = T, ci = c(50, 95))

#MCMCplot(out.1$, ref_ovl = T, ci = c(50, 95))

det.out <- out.1$alpha.samples
fitted.intMsPGOcc(out.1)
```
