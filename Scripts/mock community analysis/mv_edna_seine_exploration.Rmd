...
indic.sign.bh <- indic.sign[p.value.bh <=0.05] %>%
  ...
  arrange(index)
unique(indic.sign.bh$index) # 4, 6, 7, 11, 14, 16, 17

# eelg.edna <- filter(indic.sign.bh, index == 4) 
# kelp.seine <- filter(indic.sign.bh, index == 6)
# mixed.seine <- filter(indic.sign.bh, index == 7)
# eelg.mixed.seine <- filter(indic.sign.bh, index == 11)
# kelp.mixed.seine <- filter(indic.sign.bh, index == 14)
# allhab.edna <- filter(indic.sign.bh, index == 16)
# allhab.seine <- filter(indic.sign.bh, index == 17) 
# #
```
...
# next pull out rownames and create box plots from the real data from those groups
eelg.edna.indic <- filter(long, name %in% eelg.edna$rn) %>%
  filter(intx %in% c("eelgrass_edna")) %>%
  mutate(intx = ifelse(intx == "eelgrass_edna", "Eelgrass eDNA",intx)) %>%
  ggplot() +
  geom_boxplot(mapping = aes(x = intx, y = value,
                             fill = name)) +
  labs(x = "Habitat and sampling gear", y = "Proportional abundance",
...
kelp.seine.indic <- filter(long, name %in% kelp.seine$rn) %>%
  filter(intx %in% c("kelp_seine")) %>%
  mutate(intx = ifelse(intx == "kelp_seine", "Understory kelp seine",intx)) %>%
  ggplot() +
  geom_boxplot(mapping = aes(x = intx, y = value,
                             fill = name)) +
  labs(x = "Habitat and sampling gear", y = "Proportional abundance",
...
mixed.seine.indic <- filter(long, name %in% mixed.seine$rn) %>%
  filter(intx %in% c("mixed_seine")) %>%
  mutate(intx = ifelse(intx == "mixed_seine", "Mixed eelgrass seine",intx)) %>%
  ggplot() +
  geom_boxplot(mapping = aes(x = intx, y = value,
                             fill = name)) +
  labs(x = "Habitat and sampling gear", y = "Proportional abundance",
...
mixed.seine.indic <- filter(long, name %in% eel$rn) %>%
  filter(intx %in% c("mixed_seine")) %>%
  mutate(intx = ifelse(intx == "mixed_seine", "Mixed eelgrass seine",intx)) %>%
  ggplot() +
  geom_boxplot(mapping = aes(x = intx, y = value,
