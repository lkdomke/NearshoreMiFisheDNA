...
edna.join <- edna.red %>% # this has the sites removed that don't have associated seine samples
  dplyr::select(community, alternative_ID, bay_year) %>%
  distinct() # 102 total samples- matches our site.info for edna

edna.com <- read.csv("Data/all_taxa_simple_20241219.csv") %>%
  left_join(edna.join, by = c("bottle" = "community")) %>%
  dplyr::select(-c(X, bottle))

unique(edna.com$taxon)
seine.ordered  <- seine %>%
  arrange(species_scientific)
unique(seine.ordered$species_scientific) 
```

```{r}
#' Species we'll need to fix: 
#' Anoplarchus (group seine data)
#' Combine Artedius species (to mimic seines)
#' Combine Blepsias bilobus and Blepsis cirrhosus
#' Deal with Clinocottus acuticeps and Clinocottus embryum
#' Gadus (Microgadus proximus)
#' Hexagrammos and Hexagrammos decagrammus
#' Hypomesus 
#' Micropterus (freshwater species? )
#' Combine Myoxocephalus
#' What to do wtih Pleuronectidae 1, 2, 3
#' Salvelinus (group seine data)
#' Sebastes 1, 2, 3 (by groups)
#' Sebastes entomelas (Widow rockfish)
#' Sebastes nigrocinctus (tiger rockfish)
```

Questionable species: 
Embiotoca lateralis (Striped surfperch); although shows up in Wrangell
Hypomesus (genus of smelts), includes: surf, pon, japanese, delta smelt
Micropterus (genus of freshwater fish, black bass); game fish introduced 13 known species

```{r}
Artedius <- c("Artedius fenestralis", "Artedius harringtoni",
              "Artedius lateralis", "Artedius spp.")

# Hexagrammos <- c("Hexagrammos", "Hexagrammos decagrammus", 
#                  "Hexagrammos octogrammus", "Hexagrammos lagocephalus",
#                  "Hexagrammos stelleri")
# 
# 
# Myox <- c("Myoxocephalus", "Myoxocephalus jaok", "Myoxocephalus polyacanthocephalus",
#           "Myoxocephalus quadricornis", "Myoxocephalus scorpioides",
#           "Myoxocephalus scorpius", "Myoxocephalus sinensis", 
#           "Myoxocephalus stelleri") # there are more but these are the ones I think common or that show up in the seine. 
# 
# Oncorhynchus <- c("Oncorhynchus clarkii", "Oncorhynchus gorbuscha", 
#                   "Oncorhynchus keta", "Oncorhynchus kisutch", 
#                   "Oncorhynchus tshawytscha", "Oncorhynchus nerka",
#                   "Oncorhynchus")
# 
# Salvelinus <- c("Salvelinus alpinus", "Salvelinus namaycush", "Salvelinus malma", 
...
# Sebastes3 <- c()
# 
# Sebastolobus <- c("Sebastolobus alascanus", "Sebastolobus altivelis",
#                   "Sebastolobus macrochir")

Stichaeidae <- c("Xiphister", "Anoplarchus insignis", "Anoplarchus",
                 "Anoplarchus purpurescens", "Lumpenus sagitta", "Stichaeus punctatus")

Hemilepidotus <- c("Hemilepidotus sp.", "Hemilepidotus hemilepidotus",
                   "Hemilepidotus spinosus")

Lepidopsetta <- c("Lepidopsetta", "Lepidopsetta bilineata", "Lepidopsetta polyxystra",
                  "Lepidopsetta mochigarei", "Lepidopsetta spp.") # northern & rock sole & dusty sole
