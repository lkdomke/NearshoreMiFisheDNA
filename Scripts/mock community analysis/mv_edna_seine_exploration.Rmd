...
for(i in unique(habitat.simper$comparison)){
  ...
  abund <- long %>%
   ...
   plot <- ggplot() +
    geom_col(aes(x = habitat, y = avg), data = abund, color = "black") +
    geom_errorbar(aes(x = habitat, ymin = avg, ymax = avg + sd), 
                  width = 0.15, data = abund) +
    geom_text(data = dat, mapping = aes(x = Inf, y = Inf, label = label), 
            inherit.aes = FALSE, hjust = 1.1, vjust = 1.4, size = 5) +
    labs(x = "Habitat", y = "Average proportion") +
         #title = unique(dat$title)) +
    plot_theme_small() +
    facet_wrap(~species_scientific)
...
edna.plots <- (habitat.plots[[1]] + habitat.plots[[2]]) / (habitat.plots[[3]]) 
edna.plots + plot_layout(axes = "collect")

h1 <- habitat.plots[[4]] 
h2 <- habitat.plots[[5]]
h3 <- habitat.plots[[6]]
test <- h1 + h2 + plot_layout(axes = "collect")
(test + h3) + plot_layout(axes = "collect")
