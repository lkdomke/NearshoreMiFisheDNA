...
pcoa <- ggplot() +
  ...
  scale_shape_manual(name = "Habitat",
                     ...
                     values = c(21, 22, 23)) +
  scale_fill_manual(name = "Sampling method",
...
# ggsave(filename = "../../In_prog_papers/In-Prog Papers/FishDetectionByTraits/Figure/Figure4_revised.png", plot = pcoa,
#       height = 8, width = 15)
```


# species accumulation curves (Fig 3)
```{r}
head(wide)
df <- data.frame()
#i <- "eelgrass_edna"
...
acc.hab.plot <- sp.accum %>%
  ...
  scale_fill_manual(name = "Sampling method & Habitat",
                     ...
                     values = c("#327610", "#327610",
                                "#E4572E", "#E4572E",
                                "#E1AD01", "#E1AD01")) +
  scale_alpha_manual(name = "Sampling method & Habitat",
                     labels = c("Eelgrass eDNA", "Eelgrass seine",
                                "Understory kelp eDNA","Understory kelp seine",
                                "Mixed eelgrass eDNA", "Mixed eelgrass seine"),
                     values = c(0.3, 1,
  ...
  scale_linetype_manual(name = "Sampling method & Habitat",
                     labels = c("Eelgrass eDNA", "Eelgrass seine",
                                "Understory kelp eDNA", "Understory kelp seine",
                                "Mixed eelgrass eDNA", "Mixed eelgrass seine"),
...
# ggsave("Images/species_accumulation_habitat_020226_2.png", dpi = 900,
#        height = 7, width = 16, plot = acc.hab.plot)

# ggsave("../../In_prog_papers/In-Prog Papers/FishDetectionByTraits/Figure/Figure3_revised.png", dpi = 600,
#         height = 7, width = 16, plot = acc.hab.plot)
```
...
heatmapdat <- indic$str %>% 
  ...
  mutate(nested = ifelse(nested == "Both gears", "Both methods", nested))

isa.plot <- 
  heatmapdat %>%
  mutate(rn = ifelse(rn == "Cottus_Leptocottus", "Cottus/Leptocottus", rn)) %>%
  ggplot() +
  geom_tile(#data = heatmapdat,
            aes(x = as.factor(comb), y = reorder(rn, -stat), fill = stat)) +
  geom_tile(inherit.aes = F, data = {heatmapdat %>% 
      # this is hella messy but it prevents left join issues below
      mutate(rn = ifelse(rn == "Cottus_Leptocottus", "Cottus/Leptocottus", rn)) %>%
      filter(signif == TRUE)},
  ...
  labs(x = "Sampling method & Habitat combinations",
...
# ggsave(plot = isa.plot, filename = "Images/updated_isa_plot_03-09-26.png", width = 16, height = 8)

# ggsave(plot = isa.plot, filename = "../../In_prog_papers/In-Prog Papers/FishDetectionByTraits/Figure/Figure5_revised_030926.png", width = 18, height = 8, dpi = 300)
#ggsave(plot = isa.plot, filename = "Images/isa_fig3_1-29-26.png", width = 16, height = 8)
```


## ISA sp in seine/eDNA w/ abund (Fig S5)
```{r}
head(seine.corrected2)
head(dang.comb)
head(heatmapdat)
isa.sp <- filter(heatmapdat, nested == "Both methods" | nested == "Seine" |
                   nested == "eDNA") %>%
  distinct(rn)
...
abundheatdat <- expand_grid(isa.sp, unique(tmp.seine.dat$comb)) %>%
  ...
  mutate(corrected_sp = ifelse(corrected_sp == "Cottus_Leptocottus", "Cottus/Leptocottus", corrected_sp)) %>%
  mutate(sum = ifelse(is.na(avg), 0, avg)) %>%
   mutate(comb = factor(comb,
          ...
          corrected_sp = factor(corrected_sp, levels = c("Artedius spp.","Hexagrammos",
                                                         ...
                                                         "Syngnathus leptorhynchus", "Cottus/Leptocottus",
...
#ggsave("Images/Abund&RelAbund_ISAsp_1-29-26.png", width = 14, height = 8, plot = raw.isa.plot)

#ggsave("Images/FigureS5_030926.png", width = 14, height = 8, plot = raw.isa.plot)
```
